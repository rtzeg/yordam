...
import { psychologistsMock } from "../psychologists/mockPsychologists";
...
export function PsychologistsListPage() {
  ...
  const handleFilterChange = (key, option) => {
    ...
    setVisibleCount(6); // сбрасываем пагинацию при изменении фильтра
  };
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    let list = psychologistsMock.filter((p) => {
      // по поиску (имя, теги, подход, язык, опыт, темы и т.п.)
      if (query) {
        const haystack = [
          p.name,
          p.therapyType,
  ...
  return (
    <MainLayout>
      <div className="mx-auto max-w-[1296px] px-4 lg:px-0 py-10">
        ...
        {/* Хедер: хлебные крошки + поиск + сортировка */}
        ...
        {/* Основной контент: фильтры + карточки */}
